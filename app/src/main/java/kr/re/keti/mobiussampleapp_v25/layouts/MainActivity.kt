...
import kr.re.keti.mobiussampleapp_v25.layouts.DeviceControlActivity.Companion
...
class MainActivity : AppCompatActivity() {
    ...
    private fun createPresMQTT(mqttStart: Boolean, serviceAEName: String) {
        if (mqttStart && presMqttClient == null) {
            ...
            try {
                val token = presMqttClient!!.connect(MqttConnectOptions())
    ...
    private val presIMqttActionListener: IMqttActionListener = object : IMqttActionListener {
        override fun onSuccess(asyncActionToken: IMqttToken) {
            ...
            Log.d(TAG, "${message}")
            try {
                presMqttClient!!.subscribe(viewModel.mqttReqTopic, mqttQos)
            } catch (e: MqttException) {
                e.printStackTrace()
            }
        }
    ...
    private val presMqttCallback: MqttCallback = object : MqttCallback {
        ...
        override fun messageArrived(topic: String, message: MqttMessage) {
            ...
            try {
                presMqttClient!!.publish(viewModel.mqttRespTopic, res_message)
            } catch (e: MqttException) {
                Log.d(TAG, "${e.message}")
            }
        }
